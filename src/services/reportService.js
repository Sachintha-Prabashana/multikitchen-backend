...
export const getAggregatedAnalytics = async () => {
  ...
  recentTransactions.forEach(t => {
    ...
    if (trendData[dateStr]) {
      if (t.type === 'ISSUE') trendData[dateStr].issues += t.quantity;
      else trendData[dateStr].receives += t.quantity;
    }
  });
  ...
  topTransactions.forEach(t => {
    if (!movementMap[t.item_id]) movementMap[t.item_id] = { name: t.item.item_name, count: 0 };
